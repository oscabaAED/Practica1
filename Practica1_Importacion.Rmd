...
library(hms)
#1
# Con la herramienta de importación
library(readr)
f2 <- read_delim("data/DFP1_11122015 124700.csv", 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE)
#View(DFP1_11122015_124700) comentamos esta línea para que no se abra en una ventana adicional

#2
f2 <- read.csv(fname, stringsAsFactors = F, sep = "\t") #añadimos sep = "\t" para que interprete bien las separaciones

#3
f2_2 <- f2

f2_2$HORA <- as_hms(f2_2$HORA)
f2_2$FECHA <- as.POSIXlt(f2_2$FECHA, format = "%d/%m/%Y")

#4
save(file="./data/DFP1.RData", list=c("f2_2"))
```
...
#1+2
library(readr)
f3 <- read_csv("data/BreadBasket_DMS.csv", 
    col_types = cols(Date = col_datetime(format = "%Y-%m-%d"), 
        Time = col_time(format = "%H:%M:%S"), 
        Transaction = col_integer()))
#View(BreadBasket_DMS) ignoramos View

#3
save(file="./data/BreadBasket.Rdata", list=c("f3"))
...
f4 <- read_sav(filename)
save(file="./data/PsychBike.Rdata", list=c("f4"))
```
...
#2
f5 <- read_json(filename, simplifyVector = T) # simplifyVector permite observar f5 como un dataframe normal
f5

#3
f5_2 <- read_file(filename)
fromJSON(f5_2)

#4
save(file="./data/FileCodificado.Rdata", list=c("f5"))
```
...
#2
iris_json <- toJSON(iris, pretty = T) # con pretty la estructura de diccionario del JSON es más fácil de leer
write_file(iris_json, "./data/IRIS_json.json")
```
...
df2 <- df[which(df[,"IDENTIFICA"] != "IDENTIFICA"),] 
# Columnas date: FECHA*
...
```{r}
#1
f7_1 <- read_excel(filename, sheet = 1)
f7_2 <- read_excel(filename, sheet = 2)
f7_3 <- read_excel(filename, sheet = 3)
f7_4 <- read_excel(filename, sheet = 4)
...
df <- list()
for (n in sheets){
  df[[n]] <- read_excel(filename, sheet = n)
}
...
#4
save(file="./data/subjectInfo.Rdata", list=c("df"))
```
